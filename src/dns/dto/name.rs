...
impl Name {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        let mut bytes = self.labels.iter().map(|l| l.serialize()).flatten().collect::<Vec<u8>>();
        bytes.push(0);
...
impl fmt::Display for Name {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.labels.iter().map(|l| l.to_string()).collect::<Vec<String>>().join("."))
    }
}

impl From<&str> for Name {
    fn from(value: &str) -> Self {
        Name{ labels: value.split(".").into_iter().map(|l| Label::from(l)).collect::<Vec<Label>>() }
    }
}
