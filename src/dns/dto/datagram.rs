...
impl Datagram {
    ...
    pub fn serialize(&self) -> Box<[u8]> {
        ...
        for i in 0..self.header.qdcount {
            let question = self.questions.get(i as usize).expect("Question index does not exist.");
            bytes.extend_from_slice(&question.serialize());
        }

        for i in 0..self.header.ancount {
            let answer = self.answers.get(i as usize).expect("Answer index does not exist.");
            bytes.extend_from_slice(&answer.serialize());
        }

        for i in 0..self.header.nscount {
            let authority = self.authorities.get(i as usize).expect("Authority index does not exist.");
            bytes.extend_from_slice(&authority.serialize());
        }

        for i in 0..self.header.arcount {
            let additional = self.additionals.get(i as usize).expect("Question index does not exist.");
...
mod tests {
    ...
    fn basic_serialize_test() {
        let datagram_bytes = [
            ...
            // question 1
            3, b'w', b'w', b'w', 6, b'g', b'o', b'o', b'g', b'l', b'e', 3, b'c', b'o', b'm', 0x00, 0x00, 0x01, 0x00, 0x01,
            // answer 1
            0xC0, 0x0C, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x68, 0x00, 0x04, 127, 0, 0, 1
        ];
        let datagram = Datagram::unserialize(&datagram_bytes);
        
        assert_eq!(datagram.header.serialize(), [0x44, 0x44, 0b10000000, 0b00000000, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00]);
        assert_eq!(*datagram.questions[0].serialize(), [3, b'w', b'w', b'w', 6, b'g', b'o', b'o', b'g', b'l', b'e', 3, b'c', b'o', b'm', 0x00, 0x00, 0x01, 0x00, 0x01]);
        assert_eq!(*datagram.answers[0].serialize(), [3, b'w', b'w', b'w', 6, b'g', b'o', b'o', b'g', b'l', b'e', 3, b'c', b'o', b'm', 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x01, 0x68, 0x00, 0x04, 127, 0, 0, 1]);
    }
}